    ...
    <div class="max-w-4xl mx-auto" >
        ...
        <div class="flex items-center mb-6" >
            <a href="{{ route('addons.index') }}" class="flex items-center text-accent hover:text-accent-dark transition-colors duration-200 mr-4" ><svg class="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <h1 class="text-2xl font-bold text-primary-text" >Edit Addon</h1>

            </div>

            <div class="bg-card-bg rounded-lg shadow-sm p-6" >

                <form action="{{ route('addons.update', $addon->
                    addon_id) }}" method="POST"> @csrf @method('PUT')
                    <div class="mb-6" >
                        ...
                        <input type="text" id="addon_name" name="addon_name" value="{{ old('addon_name', $addon->
                            addon_name) }}" required class="w-full px-4 py-2 border @error('addon_name') border-red-500
                            @else
                            border-border-color @enderror rounded-md focus:outline-none focus:ring-accent focus:border-accent" placeholder="e.g. Additional Hour"> @error('addon_name')
                    ...
                    <div class="mb-6" >
                        ...
                        <div class="relative" >
                            ...
                            <input type="number" id="addon_price" name="addon_price" value="{{ old('addon_price', $addon->
                                addon_price) }}" required min="0" step="0.01" class="w-full pl-8 pr-4 py-2 border @error('addon_price') border-red-500
                                @else
                    ...
                    <div class="mb-6" >
                        ...
                        @else
                        border-border-color @enderror rounded-md focus:outline-none focus:ring-accent focus:border-accent" placeholder="Enter addon description">{{ old('description', $addon->description) }}</textarea>
                        @error('description')
                        <p class="mt-1 text-sm text-red-600" >{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center mb-6" >
                    ...
                    <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $addon->
                    ...
                    <div class="flex justify-end space-x-3" >
                        ...
                        <button type="submit" class="px-4 py-2 bg-accent hover:bg-accent-dark text-white rounded-md transition-colors duration-150" > Update Addon </button>
